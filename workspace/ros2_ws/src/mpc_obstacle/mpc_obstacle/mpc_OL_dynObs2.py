...
class QP:
    def __init__(self, A_d, B_d, Q, R, QN,Safezone, Nc,Np, nx, nu, Ts,solver_opts=None):
        ...
        for k in range(Np):
            ...
            F = ca.vertcat(
                    ...
                    ca.horzcat(-m,  1,  0, 0, 0, 0)   # y <= mx +b -> y - mx <= b
                )
            G = ca.vertcat(w/2, w/2, -cI, xmax, -xmin,b)
            g.append(F @ xk - G) 
    ...
    def solveMPC(self,x_current, x_ref,z0,cS_val, cI_val, W_val, xmax_val,x_min_val,m_val,b_val):
        ...
        P_val = np.concatenate([x_current,x_ref,np.array([cS_val, cI_val, W_val, xmax_val, x_min_val,m_val,b_val])])
